...
bool fun(info_backup_file i, std::atomic<bool>& b) {
    ...
    int sock;
    client.serverConnection(sock);

    if (i.status == FileStatus::renamed)
    {
        std::string rename = getFileName(i.adding_info);
        
            resCode = client.sendToServer(sock, operation, path, name, rename, i.file_size, i.file_hash, 0, b);
            ...
            resCode = client.sendToServer(sock, operation, path, name, "", i.file_size, i.file_hash, 0, b);
...
void FileWatcher::start(const std::function<void(std::string, FileStatus)>& action) {
    while (running_) {
        ...
        */
        ...
        //se è la prima volta che gira il programma (runs==0) tratta tutti i file del path come nuovi file da inviare in backup
        //DA MIGLIORARE
        if (runs == 0) {
            for (auto& file : std::filesystem::recursive_directory_iterator(path_to_watch)) {
            ...
            // (1) Se un task è terminato aggiorno  la tabella dei thread, la tabella booleani ed eventualmente elements_to_backup3,
            for (auto thr = thread_table.begin(), next_thr = thr; thr != thread_table.end(); thr = next_thr) {
                ...
                if (thr->second.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
                    if (thr->second.get() == true || thread_to_stop[thr->first].load()) {
                    ...
                    if (!contains(file_name)) {
                        ...
                        while (j != paths_.end()) {
                            if (hash_to_check == j->second.file_hash && file.file_size() == j->second.file_size && !std::filesystem::exists(j->first)) {
                                ...
                                if (thread_table.find(j->first) != thread_table.end() /*thread_table.contains(j->first)*/) { //se c'è un thread che sta lavorando quel file gli si dice di smetterla e poi BISOGNA NON AGGIORNARE IL FILEWATCHER PERCHE' AL GIRO SUCCESSIVO SI DEVE DI NUOVO ACCORGERE DI QUESTO EVENTO
                                    thread_to_stop[j->first] = true;
                                    break; // <- questo perché il file watcher non deve accorgersi del nuovo file
                                }
                                ...
                                if (elements_to_backup3.find(j->first) != elements_to_backup3.end() /*elements_to_backup3.contains(j->first)*/) {
                                    ...
                                    while (it != elements_to_backup3.end()) {
                                        if (it->second.status == FileStatus::renamed && it->second.adding_info == j->first && it->first != file_name) {
                                            ...
                                            if (thread_table.find(it->first) != thread_table.end() /*thread_table.contains(it->first)*/) { //se c'è un thread che sta lavorando quel file gli si dice di smetterla
                        ...
                        if (count == paths_.size()) {
                            ...
                            if (thread_table.find(file_name) != thread_table.end() /*thread_table.contains(file_name)*/) { //se c'è un thread che sta lavorando quel file gli si dice di smetterla
                                ...
                                //nulla perché è necessario che prima venga processata la rename, la new viene ignorata.
                            }
                            else {
            ...
            while (it != paths_.end()) {
                if (!std::filesystem::exists(it->first)) {
                    ...
                    if (thread_table.find(it -> first) != thread_table.end() && !thread_to_stop[it->first].load()/*thread_table.contains(it->first)*/) { //se c'è un thread che sta lavorando quel file gli si dice di smetterla
                        thread_to_stop[it->first] = true;
                        it++;
                        ...
                        if (elements_to_backup3.find(it->first) != elements_to_backup3.end() /*elements_to_backup3.contains(it->first) */&& elements_to_backup3[it->first].status == FileStatus::created) {
                            ...
                            while (iter != elements_to_backup3.end()) {
                                if (iter->second.adding_info == it->first && iter->second.status == FileStatus::renamed) {
                                    if (thread_table.find(iter->first) != thread_table.end() && !thread_to_stop[iter->first].load() /*thread_table.contains(iter->first)*/) { //se c'è un thread che sta lavorando quel file gli si dice di smetterla
